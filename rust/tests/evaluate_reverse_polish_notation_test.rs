use algo_leetcode::stack_and_line::evaluate_reverse_polish_notation::Solution;
...
fn test_division_truncation() {
    assert_eq!(Solution::eval_rpn(vec!["7".to_string(), "2".to_string(), "/".to_string()]), 3);
    assert_eq!(Solution::eval_rpn(vec!["-8".to_string(), "3".to_string(), "/".to_string()]), -2);
}
...
fn test_negative_numbers() {
    assert_eq!(Solution::eval_rpn(vec!["3".to_string(), "-4".to_string(), "*".to_string()]), -12);
    assert_eq!(Solution::eval_rpn(vec!["-6".to_string(), "-2".to_string(), "/".to_string()]), 3);
}